    ...
    <div class="container mx-auto max-w-2xl">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                ...
                <p class="mt-2 text-sm text-gray-700">Manage the fields linked to the subscriber.</p>
            </div>
        ...
        <div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg" v-if="fields.length && subscriber.fields">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="relative py-3.5 pl-3 pr-4 sm:pr-6">
                            <span class="sr-only">Link field</span>
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 bg-white">
                    <tr v-for="field in fields" :key="field.id">
                        ...
                        <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <button v-if="subscriber.fields.find(item => item.id === field.id)" @click="deleteSubscriberField(field)" class="text-red-600 hover:text-red-900">Unlink field<span class="sr-only">, {{ field.title }}</span></button>
                            <button v-else @click="addSubscriberField(field)">Link field<span class="sr-only">, {{ field.title }}</span></button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
import { useFieldsStore, useSubscribersStore } from '@/stores';
...
const deleteSubscriberField = (field) => {
  subscribersStore.deleteField(field.id);
};

const addSubscriberField = (field) => {
